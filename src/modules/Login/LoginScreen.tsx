        ...
        <div style={{display:'flex',gap:'clamp(16px,4vw,64px)',alignItems:'center',maxWidth:960,width:'100%',flexWrap:'wrap',justifyContent:'center'}}>
          ...
          <div style={{background:'rgba(255,255,255,0.04)',backdropFilter:'blur(20px)',border:'1px solid rgba(255,255,255,0.1)',borderRadius:20,padding:'36px 32px',width:360,flexShrink:0,boxShadow:'0 25px 60px rgba(0,0,0,0.4)'}}>
            ...
            <button onClick={attempt} disabled={loading||!uid.trim()}
              style={{width:'100%',padding:'12px',borderRadius:10,border:'none',cursor:'pointer',background:loading?'#334155':'linear-gradient(135deg,#2563eb,#4f46e5)',color:'white',fontSize:13,fontWeight:700,boxShadow:loading?'none':'0 4px 14px rgba(37,99,235,0.45)',transition:'all 0.15s',opacity:uid.trim()?1:0.6}}>
              {loading?'Signing in…':'Sign In →'}
            </button>
            <div style={{textAlign:'center',marginTop:16,color:'#475569',fontSize:11}}>Demo credentials pre-filled · Read-only mode available</div>
          </div>
        </div>
      </div>
