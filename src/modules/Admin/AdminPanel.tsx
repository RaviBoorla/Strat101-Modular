  ...
  return(
    <div>
      <div style={{display:'flex',justifyContent:'flex-end',gap:6,marginBottom:14}}>
        <button onClick={()=>onUpdate({...tenant,features:{kanban:true,workitems:true,create:true,bot:true,reports:true,ride:false}})} style={{padding:'4px 10px',borderRadius:6,border:'1px solid #bbf7d0',background:'#f0fdf4',color:'#16a34a',fontSize:11,fontWeight:600,cursor:'pointer'}}>All On</button>
        <button onClick={()=>onUpdate({...tenant,features:{kanban:false,workitems:false,create:false,bot:false,reports:false,ride:false}})} style={{padding:'4px 10px',borderRadius:6,border:'1px solid #fecaca',background:'#fef2f2',color:'#dc2626',fontSize:11,fontWeight:600,cursor:'pointer'}}>All Off</button>
      </div>
      {FEATURE_DEFS.map(fd=>{
...
export default function AdminPanel({loggedUser,onPreviewTenant,embedded=false}:AdminPanelProps){
