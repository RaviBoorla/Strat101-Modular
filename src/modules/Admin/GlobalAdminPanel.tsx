...
const FEATURE_LABELS: Record<string,string> = {
  ...
  bot:'AI Assist', reports:'Reports', ride:'RiDe Intel', chat:'Team Chat',
  sprints:'Sprints & Backlog', agentSprints:'Agent Sprints',
};
const FEATURE_DEFS: { key: FeatureKey; label: string; icon: string }[] = [
  ...
  { key:'agentSprints', label:'Agent Sprints',     icon:'AgentSprints' },
];

const FEATURE_EMOJI: Record<FeatureKey, string> = {
  kanban:'🗂️', workitems:'📦', create:'➕', bot:'🤖', reports:'📈', ride:'⚡', chat:'💬', sprints:'🏃', agentSprints:'🤖',
};
    ...
    <div>
      ...
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:10}}>
        ...
        <div style={{display:'flex',gap:6}}>
          <button onClick={()=>onUpdate({...tenant,features:{kanban:true,workitems:true,create:true,bot:true,reports:true,ride:true,chat:true,sprints:true,agentSprints:true}})} style={{padding:'3px 10px',borderRadius:6,border:'1px solid #bbf7d0',background:'#f0fdf4',color:'#16a34a',fontSize:11,fontWeight:600,cursor:'pointer'}}>All On</button>
          <button onClick={()=>onUpdate({...tenant,features:{kanban:false,workitems:false,create:false,bot:false,reports:false,ride:false,chat:false,sprints:false,agentSprints:false}})} style={{padding:'3px 10px',borderRadius:6,border:'1px solid #fecaca',background:'#fef2f2',color:'#dc2626',fontSize:11,fontWeight:600,cursor:'pointer'}}>All Off</button>
        </div>
      </div>
      {FEATURE_DEFS.map(fd=>{
...
function TenantForm({tenant,onSave,onClose}:{tenant:Tenant|null;onSave:(t:Tenant)=>void;onClose:()=>void}){
  ...
  const save=()=>{
    ...
    const base={kanban:true,workitems:true,create:true,bot:true,reports:true,ride:false,chat:false,sprints:false,agentSprints:false};
...
export interface GlobalAdminPanelProps {
  loggedUser:      string;
  onPreviewTenant: (tenant: Tenant) => void;
  embedded?:       boolean;
}

export default function GlobalAdminPanel({loggedUser,onPreviewTenant,embedded=false}:GlobalAdminPanelProps){
      ...
      <div style={{display:'flex',height:'100%',overflow:'hidden',fontFamily:'system-ui,sans-serif',fontSize:13}}>
        ...
        <div style={{width:220,borderRight:'1px solid #e2e8f0',display:'flex',flexDirection:'column',background:'white',flexShrink:0}}>
          <div style={{padding:'10px 12px',borderBottom:'1px solid #f1f5f9'}}>
            <input value={tenantSearch} onChange={e=>setTenantSearch(e.target.value)}
              ...
              style={{width:'100%',boxSizing:'border-box',border:'1px solid #e2e8f0',borderRadius:7,padding:'6px 10px',fontSize:11,outline:'none'}}/>
          </div>
          <div style={{flex:1,overflowY:'auto'}}>
            <div onClick={()=>setSelectedTenant(null)}
              style={{padding:'8px 12px',cursor:'pointer',fontSize:11,fontWeight:600,color:!selectedTenant?'#2563eb':'#64748b',background:!selectedTenant?'#eff6ff':'transparent',borderBottom:'1px solid #f8fafc',display:'flex',alignItems:'center',gap:6}}>
              {pending.length>0&&<span style={{background:'#dc2626',color:'white',borderRadius:999,fontSize:9,padding:'1px 5px',fontWeight:700}}>{pending.length}</span>}
              ⚡ Approvals
            </div>
            {filteredTenants.map(t=>(
              <div key={t.id} onClick={()=>setSelectedTenant(t)}
                ...
                <div style={{fontSize:10,color:'#94a3b8',marginTop:2,display:'flex',gap:6}}>
                  <span>{t.slug}</span>
                  <span style={{color:t.active?'#16a34a':'#dc2626',fontWeight:600}}>{t.active?'Active':'Inactive'}</span>
                </div>
              </div>
            ))}
            <div onClick={()=>setEditing('new')}
              style={{padding:'8px 12px',cursor:'pointer',fontSize:11,fontWeight:600,color:'#2563eb',display:'flex',alignItems:'center',gap:4,borderTop:'1px solid #e2e8f0',marginTop:4}}>
              + New Tenant
            </div>
          </div>
          {dbLoading&&<div style={{padding:8,fontSize:10,color:'#94a3b8',textAlign:'center'}}>Loading...</div>}
        </div>
            ...
            <div style={{padding:16,flex:1,overflowY:'auto'}}>
              ...
              :<div style={{display:'flex',flexDirection:'column',gap:8}}>
                {pending.map((item:any)=>(
                  <div key={item.id} style={{background:'white',border:'1px solid #e2e8f0',borderRadius:8,padding:'10px 12px'}}>
                    <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:8}}>
                      ...
                      <div style={{display:'flex',gap:4,flexShrink:0}}>
                        {item._type==='user'&&<>
                          <button onClick={()=>approveUser(item.id)} style={{padding:'3px 8px',borderRadius:5,border:'none',background:'#16a34a',color:'white',fontSize:10,fontWeight:600,cursor:'pointer'}}>Approve</button>
                          <button onClick={()=>rejectUser(item.id)} style={{padding:'3px 8px',borderRadius:5,border:'1px solid #fca5a5',background:'white',color:'#dc2626',fontSize:10,fontWeight:600,cursor:'pointer'}}>Reject</button>
                        </>}
        ...
        {confirmDel&&(
          <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.4)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:200}}>
            <div style={{background:'white',borderRadius:12,padding:20,maxWidth:320,width:'90%'}}>
              <div style={{fontWeight:700,fontSize:13,marginBottom:8}}>Delete Tenant</div>
              <p style={{fontSize:12,color:'#374151',marginBottom:14}}>Delete <strong>{tenants.find(t=>t.id===confirmDel)?.name}</strong>? This cannot be undone.</p>
              <div style={{display:'flex',gap:8,justifyContent:'flex-end'}}>
                <button onClick={()=>setConfirmDel(null)} style={{padding:'5px 12px',borderRadius:6,border:'1px solid #e2e8f0',background:'white',fontSize:11,cursor:'pointer'}}>Cancel</button>
                <button onClick={()=>deleteTenant(confirmDel)} style={{padding:'5px 12px',borderRadius:6,border:'none',background:'#dc2626',color:'white',fontSize:11,fontWeight:600,cursor:'pointer'}}>Delete</button>
              </div>
            </div>
          </div>
        )}
  ...
  return(
    <div style={{display:'flex',flexDirection:'column',height:'100%',background:'#f8fafc',fontFamily:'system-ui,sans-serif',fontSize:13}}>
      ...
      <div style={{background:'white',borderBottom:'1px solid #e2e8f0',padding:'12px 20px',display:'flex',alignItems:'center',justifyContent:'space-between',flexShrink:0}}>
        <div>
          <div style={{fontSize:15,fontWeight:700,color:'#111827'}}>Global Admin Console</div>
