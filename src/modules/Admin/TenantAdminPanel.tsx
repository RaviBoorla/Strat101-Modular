...
export default function TenantAdminPanel({ loggedUser, tenantId, onSignOut }: TenantAdminPanelProps) {
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',height:'100vh',background:'#f8fafc',fontFamily:'system-ui,sans-serif',fontSize:13}}>

      {/* ── Top bar ── */}
      <div style={{background:'#1e293b',padding:'0 20px',height:48,display:'flex',alignItems:'center',justifyContent:'space-between',flexShrink:0}}>
        <div style={{display:'flex',alignItems:'center',gap:10}}>
          ...
          <div>
            ...
            <span style={{color:'#64748b',fontSize:11,marginLeft:8}}>Tenant Admin Console</span>
          </div>
        </div>
            ...
            <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:12}}>
              <div style={{flex:1}}>
                <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:4}}>
                  <span style={{fontSize:16}}>{u.role==='tenant_admin'?'👑':u.role==='editor'?'✏️':'👁️'}</span>
...
function AddUserModal({ tenantId, tenantName, onClose, onSaved }:
  ...
  const [role,     setRole]     = useState<'tenant_admin'|'editor'|'viewer'>('editor');
