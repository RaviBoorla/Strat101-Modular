...
function buildSystemPrompt(items: any[]) {
  const summary = items.map(it => ({
    ...
    impact: it.impact || "",
    tags: it.tags, links: it.links, dependencies: it.dependencies,
    ...
    <div className="flex flex-col h-full" style={{ fontFamily:"system-ui,sans-serif", background:'#f1f5f9' }}>
          ...
          <div className="space-y-4" style={{ maxWidth:760, margin:"0 auto" }}>
            {messages.map((m, i) => (
              <div key={i} className={`flex gap-3 ${m.role==="user" ? "justify-end" : "justify-start"}`}>
                ...
                <div style={{borderRadius:16,padding:'10px 16px',fontSize:13,lineHeight:1.65,maxWidth:'80%',whiteSpace:'pre-wrap',...(m.role==="user"?{background:'#2563eb',color:'white',borderTopRightRadius:4}:{background:'white',color:'#1e293b',border:'1px solid #e2e8f0',borderTopLeftRadius:4,boxShadow:'0 1px 4px rgba(0,0,0,0.06)'})}}>
                  {m.content}
                </div>
                {m.role === "user" && (
            ...
            {loading && (
              <div className="flex gap-3 justify-start">
                ...
                <div style={{ background:'white',border:'1px solid #e2e8f0',borderRadius:16,borderTopLeftRadius:4,padding:'12px 16px',display:'flex',alignItems:'center',gap:6,boxShadow:'0 1px 4px rgba(0,0,0,0.06)' }}>
                  {[0,1,2].map(d => (<div key={d} style={{ width:7,height:7,borderRadius:'50%',background:'#93c5fd',animation:'bounce 1.2s infinite',animationDelay:`${d*0.2}s` }} />))}
                </div>
              </div>
            )}
            {error && (<div style={{ borderRadius:12,border:'1px solid #fca5a5',background:'#fef2f2',padding:'10px 16px',color:'#dc2626',fontSize:12 }}>⚠ {error}</div>)}
            <div ref={bottomRef} />
          </div>
        )}
      </div>
      ...
      <div className="shrink-0 px-3 py-2 border-t" style={{ background:'white', borderColor:'#e2e8f0' }}>
        <div className="flex gap-2 items-end" style={{ maxWidth:760, margin:"0 auto" }}>
          <textarea ref={inputRef} value={input} onChange={e => setInput(e.target.value)} onKeyDown={onKey} rows={1} disabled={loading}
